...
// main.ts
__export(exports, {
  default: () => EnglishLearningPlugin
});
var import_obsidian = __toModule(require("obsidian"));

// gaokao_vocabulary.ts
var GAOKAO_WORDS = new Set([
  "a",
  "abandon",
  "ability",
  "able",
...
var EnglishLearningPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.popup = null;
  ...
  async onload() {
    ...
    this.registerDomEvent(document, "mousedown", (evt) => {
      if (this.popup && !this.popup.contains(evt.target)) {
        this.closePopup();
      }
    });
  ...
  parseExclusionList() {
    this.customExclusion = new Set(this.settings.exclusionList.split(",").map((w) => w.trim().toLowerCase()).filter((w) => w.length > 0));
  }
  closePopup() {
    if (this.popup) {
      this.popup.remove();
  ...
  highlightWords(element) {
    ...
    while (node = walker.nextNode()) {
      ...
      if (parent && (parent.tagName === "CODE" || parent.tagName === "PRE" || parent.tagName === "A" || parent.classList.contains("dict-word-clickable") || parent.closest(".dict-card"))) {
        continue;
      }
      ...
      parts.forEach((part) => {
        if (/^[a-zA-Z]{2,}$/.test(part)) {
          ...
          const isCommon = GAOKAO_WORDS.has(lower) || this.customExclusion.has(lower) || lower.endsWith("s") && GAOKAO_WORDS.has(lower.slice(0, -1)) || lower.endsWith("es") && GAOKAO_WORDS.has(lower.slice(0, -2)) || lower.endsWith("ed") && GAOKAO_WORDS.has(lower.slice(0, -2)) || lower.endsWith("ed") && GAOKAO_WORDS.has(lower.slice(0, -1)) || lower.endsWith("ing") && GAOKAO_WORDS.has(lower.slice(0, -3)) || lower.endsWith("ing") && GAOKAO_WORDS.has(lower.slice(0, -3) + "e");
          if (!isCommon) {
            span.addClass("word-highlight");
          }
          span.onclick = (e) => {
            e.stopPropagation();
            e.preventDefault();
    ...
    nodesToReplace.forEach(({ node: node2, fragment }) => {
      try {
        if (node2.parentNode) {
  ...
  async handleSelection(evt) {
    ...
    if (!text || !/^[a-zA-Z]+$/.test(text) || text.length < 2 || text.length > 40)
      return;
    ...
    try {
      ...
      if (data.blng_sents_part && data.blng_sents_part["sentence-pair"]) {
        ...
        if (pair.sentence && pair["sentence-translation"]) {
          exampleEn = pair.sentence.replace(/<\/?[^>]+(>|$)/g, "");
    ...
    try {
      if (!file) {
        ...
        if (!content.includes("| \u5355\u8BCD |")) {
          await this.app.vault.modify(file, header + content + row);
        } else {
          await this.app.vault.append(file, row);
        }
      }
      new import_obsidian.Notice(`\u5DF2\u5B58\u5165\u751F\u8BCD\u672C: ${data.word}`);
    } catch (err) {
      new import_obsidian.Notice("\u4FDD\u5B58\u5931\u8D25\uFF0C\u8BF7\u68C0\u67E5\u6587\u4EF6\u8DEF\u5F84\u8BBE\u7F6E");
      console.error(err);
    }
  }
};
var EnglishLearningSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Vocabulary Book Path").setDesc("Path to your vocabulary markdown file (e.g., Folder/Vocabulary.md)").addText((text) => text.setValue(this.plugin.settings.vocabularyBookPath).onChange(async (value) => {
      this.plugin.settings.vocabularyBookPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Auto Spoken").setDesc("Automatically read word on selection").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoRead).onChange(async (value) => {
      this.plugin.settings.autoRead = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Exclusion List").setDesc("Comma separated words to ignore (e.g. apple, banana)").addTextArea((text) => text.setValue(this.plugin.settings.exclusionList).onChange(async (value) => {
